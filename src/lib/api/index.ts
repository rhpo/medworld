...
export const CabinetAPI = {
  ...
  async getAssistants(
    cabinetId: number
  ): Promise<Array<User<Users.Assistant>>> {
    await randomDelay();
    const cabinet = fakeCabinets.find((c) => c.id === cabinetId);
...
export const UserAPI = {
  ...
  async getById(userID: User<any>["id"]) {
    return (
      (await AllAPI.listAllUsers()).find((user) => user.id === userID) || null
    );
  },
};
  ...
  async getPatients(doctorId: User<any>["id"], cabinetId?: Cabinet["id"]) {
    if (typeof cabinetId === "undefined") {
      return fakePatients.filter((p) =>
        fakeAppointments.some(
          (app) => app.doctor.id === doctorId && app.patient.id === p.id
        )
      );
    } else {
      return fakePatients.filter((p) =>
