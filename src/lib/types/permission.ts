...
export const MessageManagement: Permission[] = ["send_message", "view_message"];
...
export function permissionGroupMap(
  ...
  return {
    cabinets: permissions.filter((e) => e.endsWith("_cabinet")).length > 0,
    patients: permissions.filter((e) => e.endsWith("_patient")).length > 0,
    assistants: permissions.filter((e) => e.endsWith("_assistant")).length > 0,
    doctors: permissions.filter((e) => e.endsWith("_doctor")).length > 0,
    appointments:
