...
import { AppointmentStatus, type Appointment } from "./appointment";
    ...
    ratings: [
      {
        ...
        review:
          "BEST CLINIC EVER! The staff were super friendly and the equipment was top-notch.",
        id: 1,
    ...
    ratings: [
      {
        ...
        review:
          "Bruh, the wait time was unbearable, but at least the equipment was decent.",
        id: 1,
        patient: fakeAdmins[1] as any,
      },
    ],
    ...
    ratings: [
      {
        ...
        id: 1,
        review:
          "HORRIBLE experience! The equipment was outdated and the staff were unprofessional, never goingf here again!",
        patient: fakeAdmins[1] as any,
      },
    ],
  },
    ...
    ratings: [
      {
        ...
        id: 1,
        review:
          "The equipment was good, but the professionalism could be improved.",
        patient: fakeAdmins[1] as any,
      },
    ],
  },
...
for (const doc of fakeDoctors) {
  const workCabinets = fakeCabinets.filter((c) =>
    c.doctors.some((d) => d.id === doc.id)
  );
...
export const fakeConsultations: Consultation[] = [
  {
    ...
    notes:
      "Initial consultation for hypertension and diabetes management. BP: 145/90. Blood sugar: 7.8 mmol/L (fasting). Patient reports occasional dizziness and fatigue. Physical examination normal. ECG shows mild LVH.",
    prescriptions: [
    ...
    attachments: [
      "bp_chart_20240115.pdf",
      "ecg_20240115.pdf",
      "lab_results_20240115.pdf",
    ],
  ...
  },
  {
    ...
    notes:
      "Follow-up visit. BP improved to 135/85. Blood sugar levels better controlled (avg 6.5 mmol/L). Patient adhering to medication schedule. Reports improved energy levels. Weight loss of 3kg since last visit.",
    prescriptions: [
...

  {
    ...
    notes:
      "Patient reports increased frequency of migraines (2-3x/week) and seasonal asthma symptoms. Lung examination clear. Peak flow 400 L/min. Migraine diary shows correlation with stress and poor sleep.",
    prescriptions: [
...

  {
    ...
    notes:
      "Follow-up for GERD and anxiety. Reports improvement in reflux symptoms with medication and dietary changes. Anxiety symptoms stable but still present during work hours. Sleep improved with relaxation techniques.",
    prescriptions: [
...

  {
    ...
    notes:
      "Regular follow-up for hypothyroidism and anemia. TSH levels normalized (2.8 mU/L). Hemoglobin improved to 11.8 g/dL. Patient reports better energy levels and reduced cold sensitivity. Weight stable.",
    prescriptions: [
...
for (const appt of fakeAppointments) {
  ...
  for (const assistant of fakeAssistants) {
    if (
      ...
      assistant.doctors.some((doc) => doc.id === appt.doctor.id)
    ) {
...
for (const doc of fakeDoctors) {
  ...
  if (doc.id === fakeDoctors[0].id) {
    doc.reviews.push({
      ...
      rating: 5,
      message:
        "Dr. El-miloudi is an exceptional cardiologist. He took the time to explain my hypertension condition thoroughly and created a comprehensive treatment plan. His follow-up care has been excellent. Very pleased with the improvement in my blood pressure control.",
    });
    doc.reviews.push({
      from: fakePatients[2],
      to: doc,
      rating: 4,
      message:
        "Very knowledgeable and professional doctor. The wait times can be a bit long, but the quality of care is worth it. Explains everything clearly and answers all questions patiently.",
    });
    ...
    doc.reviews.push({
      ...
      rating: 5,
      message:
        "Dr. Belaiouer has been managing my asthma and migraines wonderfully. She is very attentive and her holistic approach to treatment has made a significant difference. Her staff is also very helpful and friendly.",
    });
    doc.reviews.push({
      from: fakePatients[3],
      to: doc,
      rating: 4,
      message:
        "Great family doctor who really listens to her patients. Has been very helpful with managing my thyroid condition. Always follows up and ensures prescriptions are working well.",
    });
    ...
    doc.reviews.push({
      ...
      rating: 5,
      message:
        "Excellent pediatrician! Dr. Nekkmouche is amazing with children and very thorough in his examinations. Makes both parents and kids feel comfortable. Highly recommended!",
    });
    doc.reviews.push({
      from: fakePatients[0],
      to: doc,
      rating: 4,
      message:
        "Very good with kids, explains everything clearly to parents. The only downside is sometimes the wait can be long, but the care is excellent.",
    });
    ...
    doc.reviews.push({
      ...
      rating: 5,
      message:
        "Dr. Zerrouki is a skilled dermatologist. Very thorough in her examinations and explains treatment options clearly. Modern approach to skincare and treatment. The clinic is well-equipped.",
    });
    doc.reviews.push({
      from: fakePatients[3],
      to: doc,
      rating: 3,
      message:
        "Competent doctor but the wait times can be very long. Would appreciate better scheduling system. Treatment was effective though.",
    });
  }

  doc.messages = [];

  const doctorConsultations = fakeConsultations.filter(
    (cons) => cons.doctor.id === doc.id
  );
  const latestConsultation =
    doctorConsultations.length > 0 ? doctorConsultations[0] : undefined;
  ...
  if (doc.id === fakeDoctors[0].id) {
    if (latestConsultation) {
      doc.messages.push({
        ...
        content: {
          message:
            "Regarding patient Riad Mahrez: Blood pressure now stabilized with Lisinopril. Please monitor during regular check-ups. Will review again in 3 months.",
      ...
      doc.messages.push({
        ...
        content: {
          message:
            "Thank you for the update on Mr. Mahrez. Will monitor BP during his next visit. Also noting good diabetes control with current regimen.",
          attachment: latestConsultation,
        },
        status: "seen",
      });
      ...
      doc.messages.push({
        ...
        content: {
          message:
            "Organizing a case conference next week to discuss recent cardiovascular findings in young patients. Would value your pediatric perspective. Available Tuesday or Thursday afternoon.",
          attachment: latestConsultation,
        },
        status: "seen",
      });
      ...
      doc.messages.push({
        ...
        content: {
          message:
            "Patient with unusual skin manifestations possibly related to new BP medication. Could you provide a quick consultation? Sending photos and current medication list.",
          attachment: latestConsultation,
        },
        status: "unseen",
      });
      ...
      doc.messages.push({
        ...
        content: {
          message:
            "Initiating a study on hypertension management in diabetic patients. Would you be interested in collaborating? Have 20 potential candidates from our shared patient pool.",
          attachment: latestConsultation,
        },
        status: "unseen",
      });
  ...
  if (doc.id === fakeDoctors[1].id) {
    const familyDocConsultation = fakeConsultations.find(
      (cons) => cons.doctor.id === doc.id
    );
    if (familyDocConsultation) {
      doc.messages.push({
        ...
        content: {
          message:
            "Would appreciate your input on recent cases of respiratory infections in children. Can we schedule a brief discussion?",
      ...
      doc.messages.push({
        ...
        content: {
          message:
            "Monthly staff meeting scheduled for next Tuesday at 8:30 AM. Please review updated clinic protocols before the meeting.",
          attachment: familyDocConsultation,
        },
        status: "unseen",
      });
  ...
  if (doc.id === fakeDoctors[2].id) {
    const pediatricConsultation = fakeConsultations.find(
      (cons) => cons.doctor.id === doc.id
    );
    if (pediatricConsultation) {
      doc.messages.push({
        ...
        content: {
          message:
            "Available for discussion tomorrow at 13:00. Have noticed similar trend in my practice. Will share my observations and treatment protocols.",
      ...
      doc.messages.push({
        ...
        content: {
          message:
            "Need to update our pediatric examination protocols. Will share findings at next staff meeting.",
          attachment: pediatricConsultation,
        },
        status: "unseen",
      });
  ...
  if (doc.id === fakeDoctors[3].id) {
    const dermatologyConsultation = fakeConsultations.find(
      (cons) => cons.doctor.id === doc.id
    );
    if (dermatologyConsultation) {
      doc.messages.push({
        ...
        content: {
          message:
            "Interesting case of systemic lupus with skin manifestations. Would appreciate your input on managing the general health aspects.",
      ...
      doc.messages.push({
        ...
        content: {
          message:
            "Several appointment requests for next week. Should we extend clinic hours on Wednesday?",
          attachment: dermatologyConsultation,
        },
        status: "unseen",
      });
